...
*/
   public static void main(String[] args) {
    ...
    public static void insert(String zw, String yw, String code) {
        String sql = "update  huizong set 中文名=?,英文名=? where 股票代码=?";
        try {
            PreparedStatement statement = MysqlUtils.getConnection().prepareStatement(sql);
            statement.setString(1, zw);
            statement.setString(2, yw);
            statement.setString(3, code);
            statement.execute();
        } catch (Exception e) {
            System.out.println("公司为"+zw);
            e.printStackTrace();
        }
    }
}
